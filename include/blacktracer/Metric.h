...
namespace metric {
	static double starta = 0.0;
	static double startasq = 0.0;
	static double* a = &starta;
	static double* asq = &startasq;
	...
	inline double _w(double r, double theta) {
		return 2. * PA * r / sq(_Sigma(r, theta));
	};

	inline double _ro(double r, double theta) {
		return sqrt(sq(r) + PAsq*sq(cos(theta)));
	};

	static double _rosq(double r, double theta) {
		return sq(r) + PAsq*sq(cos(theta));
	};
	...
	static double _BigTheta(double r, double theta, double b, double q) {
		return q - sq(cos(theta))*(sq(b) / sq(sin(theta)) - PAsq);
	};

	inline double calcSpeed(double r, double theta) {
		double omega = 1. / (PA + pow(r, 1.5));
	...
	static double _b0(double r0) {
		return -(sq3(r0) - 3. * sq(r0) + PAsq*r0 + PAsq) / (PA*(r0 - 1.));
	};

	static double _b0diff(double r0) {
		return (PAsq + PA - 2. * r0*(sq(r0) - 3. * r0 + 3.)) / (PA*sq(r0 - 1.));
	};

	static double _q0(double r0) {
		return -sq3(r0)*(sq3(r0) - 6. * sq(r0) + 9. * r0 - 4. * PAsq) / (PAsq*sq(r0 - 1.));
	};
	...
	static bool checkB_Q(double bV, double qV) {
		double _r1 = 2. * (1. + cos(2. * acos(-PA) / 3.));
		double _r2 = 2. * (1. + cos(2. * acos(PA) / 3.));
	...
	static void derivs(double* var, double* varOut, double b, double q) {
		...
		// result: nothing good...
		dpdz = (2 * PA*P - (2 * PA - 2 * b)*delta + (2 * b*cossq * delta) / sinsq) / (rosq * 2 * delta);
		...
		dptdz = ((2 * cosv*sinv*(bsq / sinsq - PAsq) + (2 * bsq*sq3(cosv)) / sq3(sinv))*delta) /
	...
	static bool rkqs(double* var, double* dvdz, int n, double& z, double &h,
	...
	inline bool check2PIcross(const std::vector<glm::dvec2>& spl, float factor) {
	...
	inline bool check2PIcross(const std::vector<glm::vec2>& spl, float factor) {
		for (int i = 0; i < spl.size(); i++) {
			if (spl[i]_phi > PI2*(1. - 1. / factor))
				return true;
		}
	...
	/// <param name="poss">The coordinates of the polygon corners.</param>
	/// <param name="factor">The factor to check whether a point is close to the border.</param>
	inline bool correct2PIcross(std::vector<glm::dvec2>& spl, float factor) {
		bool check = false;
		for (int i = 0; i < spl.size(); i++) {
	...
	inline bool correct2PIcross(std::vector<glm::vec2>& spl, float factor) {
		bool check = false;
		for (int i = 0; i < spl.size(); i++) {
			if (spl[i]_phi < PI2*(1. / factor)) {
				spl[i]_phi += PI2;
