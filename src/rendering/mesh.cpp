...
Mesh::Mesh(std::vector<glm::vec3> pos, std::vector<glm::vec2> uv, std::vector<int> indxs)
	: indices_(indxs){
	if (pos.size() != uv.size()) {
...
Mesh::Mesh(std::vector<Vertex> verts, std::vector<int> indxs)
...
void Mesh::draw(int drawMode) const {
	...
	glDrawElements(drawMode, indices_.size(), GL_INT, 0);
	glBindVertexArray(0);
}
...
void Mesh::createMesh() {
	...
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)offsetof(Vertex, uvCoord));
